...
class AgentScenarioTestSupportTest {
    ...
    fun `scenario DI keeps skills and host integrations hermetic`() = runTest {
        ...
        val getSkills: LLMToolSetup = di.direct.instance(tag = SkillToolBindingTags.GET_SKILLS_TOOL)
        val meta = ToolInvocationMeta(userId = "scenario-test")
        val getSkillsResult = getSkills.invoke(
            LLMResponse.FunctionCall(name = getSkills.fn.name, arguments = emptyMap()),
            meta,
        )
        val skillIds = restJsonMapper.readTree(getSkillsResult.content)
            .path("results")
            .map { it.path("skillId").asText() }
