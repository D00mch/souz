...
- Build one immutable request-scoped catalog by applying each execution's enabled-tool snapshot to compiled direct tools before adding client tools.
- Build every turn with the backend's single request-scoped steerable direct-tool graph.
