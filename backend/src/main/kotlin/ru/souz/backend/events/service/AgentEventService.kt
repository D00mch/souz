...
class AgentEventService(
    ...
    suspend fun appendDurable(
        ...
        require(type != AgentEventType.ASSISTANT_MESSAGE) { "Assistant progress is live-only" }
    ...
    suspend fun publishLive(
        ...
        val discardAfterSeq = if (type == AgentEventType.ASSISTANT_MESSAGE) {
            eventRepository.latestSeq(userId, chatId).also { currentCoroutineContext().ensureActive() }
        } else null
        val event = AgentLiveEvent(
            ...
            payload = payload,
            createdAt = createdAt,
            discardAfterSeq = discardAfterSeq,
        )
