...
Client-Souz executions (`clientToolsEnabled`) enable `automaticMemoryRecall` only when `SOUZ_FEATURE_WS_AUTOMATIC_MEMORY_RECALL=true` (JVM property `souz.backend.feature.wsAutomaticMemoryRecall`, default `false`). With `HINDSIGHT_API_URL` configured, enable it when the backend should retrieve memory before the first LLM call without client payload changes. Leave it disabled when clients already supply memory. Recall runs once in initial execution setup; tool loops, duplicate submissions, reconnects and active-thread continuation do not repeat it. Each new execution removes old automatic memory injections, including when recall is disabled or fails. Explicit `SearchMemory` and capture remain available independently; non-client executions retain automatic recall. Client system instructions carry the unsupported-memory-deletion notice independently of recall.
