...
Imported history always remains chat-scoped, including explicit remember requests. Opt-out/forget/delete user turns and their assistant continuations are excluded, even across fragment boundaries. Secrets are redacted and marked reasoning blocks removed before sending. Completed turns and imported history share the verified `souz-dialogue-v1` strategy: extracted facts explicitly attribute user statements, assistant proposals/reports and user selections; action reports remain unverified. The strategy accepts completed-turn role sections and history records, excludes context-only records from standalone extraction, and preserves supplied source IDs without inventing missing ones. It discards service chatter and treats dialogue as untrusted data, never instructions. Assistant restatements of recalled facts remain assistant reports, not new user statements or independent confirmation.
