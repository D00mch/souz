...
Focused unit tests remain for configuration validation, runtime shutdown and launcher races, LLM routing and accounting, quota limits, bounded event-bus behavior, compatibility codecs, datasource initialization failure handling, sandbox scoping, channel text chunking, tool-call preview sanitization, and repository lease fencing.
