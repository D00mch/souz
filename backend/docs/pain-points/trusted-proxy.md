...
- Pass `ToolInvocationMeta.userId` into backend runtime work.
- Keep the compiled backend tool catalog limited to the explicit filesystem-free allowlist. File, image, data-processing, and command-backed tools remain unavailable until a remote sandbox exists.
- Build the request-scoped execution catalog by restricting compiled tool-backed Skills with the trusted user's effective `enabledTools` snapshot. Add built-in client operations only after that selection and only for Client-Souz executions.
- Keep backend Codex access server-managed. Its OAuth access token, refresh token, account ID, expiry, and version belong to one PostgreSQL credential set; do not treat the per-user provider-key `apiKey` field as a complete Codex session. Consume a deployment seed only through the durable one-time bootstrap marker, and keep ordinary loads free of configuration fallbacks so revocation cannot recreate credentials. Acquire the PostgreSQL refresh lease before reloading credentials or calling the OAuth endpoint so rotating refresh tokens are used once across backend processes.
- Read immutable deployment settings once through `BackendAppConfig` and `BackendSettingsConfig`. Backend runtime code must not construct `ConfigStore`, Java Preferences, local model stores, or persistent sandbox paths.
...
Run `./gradlew :backend:test`. Cover proxy rejection and provisioning, public chat UUID validation and idempotency, WebSocket client-type and device-owner checks, OpenAPI security exceptions, PostgreSQL ownership isolation, the backend tool allowlist, and option continuation.
