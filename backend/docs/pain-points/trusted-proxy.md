...
- Keep backend Codex access server-managed. Its OAuth access token, refresh token, account ID, expiry, and version belong to one PostgreSQL credential set; do not treat the per-user provider-key `apiKey` field as a complete Codex session.
