...
- Build the request-scoped execution catalog by restricting compiled direct tools with the trusted user's effective `enabledTools` snapshot. Add built-in client operations only after that selection and only for Client-Souz executions.
