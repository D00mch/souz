...
- Configured subagent models may use another supported provider. Keep their explicit provider and raw ID through the execution API, using that provider's execution-scoped credentials and the shared retry/usage wrappers. Resolve enum selectors and legacy model defaults in the execution router only for calls without internal routing metadata. Supply the dedicated summarization model in its request; adapters do not override model IDs.
