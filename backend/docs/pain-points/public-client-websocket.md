...
`BackendClientSkills` exposes bundled resources through both an `AgentToolCatalog` adapter view and a `SkillBundleProvider` view. The adapter view is used for direct protocol-facing calls, while the provider view contributes IDs to prompt inventory. Bundled resources stay backend-owned and reviewed; do not let user-scoped file-backed Skills declare the client WebSocket transport.
