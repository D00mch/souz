...
Accepted user inputs are `messages` with `inputSeq`, source, device, request ID, and request metadata. The execution keeps the latest revision and device JSON. `BackendClientSkills` loads reviewed client bundles from `skills/client` and exposes a `SkillBundleProvider` view for ID inventory plus an `AgentToolCatalog` view for exact skill resolution. Their IDs, categories, timeouts, and instructions come from `SKILL.md`, while request-scoped adapters share one WebSocket transport and remain outside the direct agent catalog.

Client Skill IDs must not collide with ordinary direct tools; the backend fails request runtime construction if they do. Backend client bundles win exact-ID collisions with user-installed bundles while client tools are enabled. Manifest-name collisions between client bundles and user bundles are ambiguous for name-based UX lookup and do not silently shadow one provider.
