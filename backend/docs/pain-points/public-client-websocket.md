...
- Keep creation validation/provisioning shared with HTTP. Build creation acknowledgements from persisted immutable fields, including database timestamp precision, so only `duplicate` changes on retries.
