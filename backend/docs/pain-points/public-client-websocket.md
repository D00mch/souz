...
- Commit history and its receipt under the chat lock without reading or mutating execution state. After commit, notify a matching local runtime outside both database and registry locks; acknowledgement remains successful when no local runtime accepts the notification.
...
- Couple execute publication to the durable message gap through its trigger sequence. The runtime-owned history watermark is non-interrupting and sequence-deduplicated; its fixed boundary loader cannot advance the session cursor until a safe LLM boundary claims the gap.
