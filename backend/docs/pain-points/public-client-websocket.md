...
Accepted user inputs are `messages` with `inputSeq`, source, device, request ID, and request metadata. The execution keeps the latest revision and device JSON. Request-scoped client Skills load instructions from `SKILL.md` and share one WebSocket transport, so their operation IDs remain discoverable in the relevant Skills graph categories.
...
- Keep client Skills in their relevant request-scoped catalog categories rather than adding them to the Skills graph core-tool list. Describe operation payloads in `SKILL.md`, not Kotlin tool definitions.
