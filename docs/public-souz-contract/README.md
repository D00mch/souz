...
Send memory retrieval results through the same assistant `tool_call` shape, preserving the tool name, arguments and result. Do not copy tool results into text history: text is eligible for memory extraction. Completed Souz turns retain only the user's submitted text, excluding live tool results, imported tool history and assistant summaries, so retrieving a memory does not become a new source for that fact.
