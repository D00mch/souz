...
With `HINDSIGHT_API_URL` configured and `SOUZ_FEATURE_WS_AUTOMATIC_MEMORY_RECALL=true`, Souz automatically retrieves memory once before the first LLM call of each new thread, including when client tools are enabled. No payload changes are required. The flag defaults to `false`; client-supplied memory history remains available in either mode. Tool loops, duplicate submits, reconnects and active-thread continuation do not repeat automatic recall. The agent can invoke `SearchMemory` explicitly in either mode. With Hindsight enabled, accepted text history also enters background memory capture. See [external memory](../../backend/docs/pain-points/external-memory.md) for retention, acknowledgement timing, attribution, privacy and recovery rules.
