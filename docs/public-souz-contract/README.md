...
`user.ask` and `device.media.open` are tool-backed Skills whose instructions live in their `SKILL.md` resources under `backend/src/main/resources/skills`. They share one WebSocket transport; each live invocation suspends until `tool.result` or its deadline. The WebSocket feature requires `SOUZ_BACKEND_AGENT=skills`.
