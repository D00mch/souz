...
- Homebrew distribution is documented in `docs/homebrew.md`; ship Souz as a Homebrew cask backed by the notarized DMG rather than as a formula. Build both arch-specific DMGs with `build-logic/kmp-build-macos-dev.sh`, which exports them to `dest/<version>/`, then use `build-logic/prepare-homebrew-release.sh` to generate the tap cask from those files.
