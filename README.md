...
WebSocket automatic recall is opt-in: with `HINDSIGHT_API_URL` configured, set `SOUZ_FEATURE_WS_AUTOMATIC_MEMORY_RECALL=true` (JVM property `souz.backend.feature.wsAutomaticMemoryRecall`, default `false`). The backend retrieves memory once before the first LLM call, including with client tools enabled. See the [public contract](docs/public-souz-contract/README.md#commands).
