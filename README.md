...
Codex models use one server-managed OAuth session because the refresh token, account ID, and expiry must stay together. Configure `CODEX_ACCESS_TOKEN`, `CODEX_REFRESH_TOKEN`, `CODEX_ACCOUNT_ID`, and `CODEX_EXPIRES_AT` on the backend, then select a Codex alias such as `gpt-5.5` through the settings API. The backend stores rotated OAuth credentials in Postgres with application-layer encryption. Docker Compose persists them in `postgres-data`; preserve that data and use the same `SOUZ_MASTER_KEY` when recreating the backend container. The per-user provider-key endpoint does not accept `codex`.
...
WebSocket executions skip automatic recall by default. With `HINDSIGHT_API_URL` configured, set `SOUZ_FEATURE_WS_AUTOMATIC_MEMORY_RECALL=true` (or JVM property `souz.backend.feature.wsAutomaticMemoryRecall`) to retrieve memory once before the first LLM call, including when client tools are enabled. No WS payload changes are required. Leave it unset or `false` when clients supply memory themselves. Tool loops, duplicate submits, reconnects and continuation of an active thread do not repeat initial recall. Explicit `SearchMemory`, memory capture and automatic recall for non-client executions are independent of this flag.
