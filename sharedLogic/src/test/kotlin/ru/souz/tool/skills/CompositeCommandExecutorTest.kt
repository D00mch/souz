...
class CompositeCommandExecutorTest {
    ...
    fun `runs a tool step then a script step, threading a nested field between them`() = runTest {
        ...
        val result = executor.execute(
            ...
            meta = meta,
            allowedTools = setOf("device.mcp.call_tool"),
        )
    ...
    fun `a script step's JSON-shaped stdout flows into a later tool argument as a raw string, not an object`() = runTest {
        ...
        val result = executor.execute(
            ...
            meta = meta,
            allowedTools = setOf("SafeApiCall"),
        )
    ...
    fun `a tool step exception stops the chain before later steps run`() = runTest {
        ...
        val result = executor.execute(
            ...
            inputs = emptyMap(),
            meta = meta,
            allowedTools = setOf("device.mcp.call_tool", "other.tool"),
        )
    ...
    fun `a restricted tool name is rejected without invoking it`() = runTest {
        ...
        val result = executor.execute(
            ...
            inputs = emptyMap(),
            meta = meta,
            allowedTools = setOf(ToolInvokeSkill.NAME),
        )
    ...
    fun `a tool outside allowedTools is rejected without invoking it, even though it's not in RESTRICTED_TOOLS`() = runTest {
        ...
        val result = executor.execute(
            ...
            meta = meta,
            allowedTools = setOf("some.other.tool"),
        )
