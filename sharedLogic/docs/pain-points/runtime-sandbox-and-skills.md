...
- The skills-oriented graph offloads tool-result content only above 4,096 UTF-8 bytes and never re-offloads `GetKnowledge` or `SearchKnowledge` responses. A Knowledge reference remains usable until exact conversation cleanup; clearing context expires its referenced entries immediately.
