...
- Filesystem tools resolve a `RuntimeSandbox` from the current `ToolInvocationMeta`. The resolver maps invocation metadata to `SandboxScope` and may cache sandboxes by scope; tools must not cache a resolved path or sandbox for later users or conversations.
...
- `PROCESS` requires explicit `command` argv and ignores `scriptPath` and `args`. Composite scripts use interpreter runtimes only: bundle installation stores file contents without executable permissions.
- `SpawnSubagent` resolves selected tools from the host's filtered catalog and approves selected file-backed bundles through the host's existing policy. Its `ToolInvokeSkill` receives only the selected compiled tools and a bundle loader restricted to the selected bundles and owner, with no repeated approval inside the child. Parent and child share `SkillCommandExecutor`: use the existing stored bundle directory when present, otherwise the loose Skill directory. Scripts, executable permissions, and generated files retain their normal filesystem behavior. See [subagent execution](../../../agent/docs/pain-points/subagents.md).
- [Composite commands](../../../docs/composite-skill-commands.md) use the same resolver snapshot as ordinary Skill invocation, including request-scoped client tools. Never resolve steps against a global catalog or infer capabilities from invocation attributes. All tool names are checked before execution; cancellation propagates, and the command timeout bounds the whole chain. Script stdout stays text even when it contains JSON; tool content and input strings are decoded when valid JSON.
- Composite scripts receive only the remaining monotonic timeout budget. Local process waits are interruptible and run independently of stdin writes; timeout or cancellation terminates the running process and its descendants before draining output. Verify deadlines with real processes: virtual coroutine delays and mocked scripts cannot expose blocking process waits. Cancellation tests must wait for parent/child readiness; keep the test watchdog separate from the command timeout so dispatcher startup and output draining cannot masquerade as command cancellation.
- Skill discovery and subagent prompts share file-backed Skill detail and execution-schema builders. `SkillCommandExecutor.Args` defines the model-facing arguments. `SkillCommandExecutor` receives the loaded or approved bundle and its hash separately, then returns the complete `SandboxCommandResult` for later Knowledge offloading.
- `AgentCoreTools` groups `GetSkillByName`, `GetSkillsByCategory`, `GetSkillsNamesByCategory`, `GetKnowledge`, `SearchKnowledge`, `SearchMemory`, and generic `RunSkillCommand` outside `AgentToolCatalog`. `SearchMemory` is universal to both agent graphs; its `ConversationMemoryRuntime` host may report structured `memory_unavailable`. Desktop search uses the persistent owner and only global plus current-conversation session scopes. File-backed Skill detail and generic execution require shared approval before returning `SKILL.md` or running bundled commands when an approval gate is provided by the host.
...
- Portable Skill DI requires a host-supplied `ConversationKnowledgeStore`. Desktop binds the sandbox store; backend binds [PostgreSQL storage](../../../backend/docs/pain-points/conversation-knowledge.md). Both use `KnowledgeRecordCodec` for retention, versioned records, and validation; retrieval tools never select a storage implementation.
...
- Keep core Skill tools out of the catalog; graph nodes install the host-provided `AgentCoreTools`. Derive file-backed arguments from `SkillCommandExecutor.Args`; keep identity and authorization outside the model-facing type.
