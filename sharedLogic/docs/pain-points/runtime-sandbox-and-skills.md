...
- For `PROCESS`, an empty `command` falls back to the resolved bundled `scriptPath` plus literal `args`; explicit argv takes precedence. Keep this conversion in `SkillCommandExecutor` so local and Docker sandboxes receive the same validated command.
