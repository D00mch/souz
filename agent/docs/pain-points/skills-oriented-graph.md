...
`NodesSkillInventory` owns Skill inventory prompt augmentation and core-tool restriction. `SteerableChatNode` owns execution-scoped continuation boundaries. `NodesToolUseWithKnowledge` owns Knowledge-aware tool-result handling. `NodesPlain` shares history, response, and inline tool-execution helpers between `NodesCommon` and the lightweight child graph.
