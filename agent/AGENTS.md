...
- `SubagentTool` implements `LLMToolSetup`: hosts supply settings, selected tools, and instructions through `Setup`; the tool constructs the isolated context and awaits its lightweight graph in the caller's coroutine. Share host APIs and invocation identity, not graph nodes, history, or event streams. See [subagent execution](docs/pain-points/subagents.md).
