...
- `AgentExecutionKernelFactory` builds one request-scoped, steerable direct-tool graph and supports only `AgentId.GRAPH`. Preserve its setup order: history input, memory recall, direct-tool classification, configured always-available catalog tools, MCP tools, context enrichment, then LLM execution. Mid-run input returns directly to the active LLM loop without repeating setup.
