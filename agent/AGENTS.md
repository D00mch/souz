...
- Preserve the classic graph's turn setup order: history input, memory recall, classification, skill activation, MCP tools, ordinary context enrichment, then LLM execution. The skills-oriented graph recalls memory, installs its fixed core tools, and enriches context without classifying, activating legacy skills, or injecting MCP tools.
