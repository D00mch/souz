...
- Skill discovery uses compact inventory data: enabled direct tool-backed Skill IDs and escaped provider-backed Skill IDs only. `GetSkillByName` and generic `RunSkillCommand` prefer enabled compiled adapters, then load full file-backed bundles on demand and require cached or fresh approval before returning instructions or executing bundled commands. Validation identity is user-, skill-, bundle-, and policy-scoped.
