...
/**
 * Local SQLite-backed outbox for desktop telemetry events.
 *
 * Events are appended to [databasePath] before any network send attempt so the app can survive
 * restarts, temporary connectivity loss, and backend outages without dropping usage metrics.
 *
 * The database stores only pending telemetry payloads and send-attempt metadata. Successfully
 * delivered rows are deleted from this outbox.
 */
class TelemetryOutboxRepository(
    ...
    companion object {
        ...
        private const val INIT_BACKOFF_MS = 100L
    }
}
