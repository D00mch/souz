...
private const val INTERNET_SEARCH_MAX_SOURCE_SNIPPET_CHARS = 480
private const val INTERNET_SEARCH_MAX_SOURCE_TEXT_FOR_PROMPT = 4_000
private const val INTERNET_SEARCH_MAX_SOURCE_TEXT_FOR_RESCUE_PROMPT = 1_600
private const val INTERNET_SEARCH_MAX_FAILED_DRAFT_CHARS = 4_000

internal fun buildInternetSearchStrategyPrompt(query: String): String = buildString {
    appendLine("User research request:")
    appendLine(query)
}.trim()

internal fun buildInternetSearchSynthesisPrompt(
    query: String,
    ...
    appendLine("Sources:")
    appendInternetSearchSourceDigest(sources, INTERNET_SEARCH_MAX_SOURCE_TEXT_FOR_PROMPT)
}.trim()

internal fun buildInternetSearchRescuePrompt(
    query: String,
    mode: ToolInternetSearch.SearchMode,
    sources: List<InternetSearchCollectedSource>,
    strategy: InternetSearchResearchStrategy?,
    ...
    failedDraft?.trim()?.takeIf { it.isNotBlank() }?.let { raw ->
        ...
        appendLine(raw.take(INTERNET_SEARCH_MAX_FAILED_DRAFT_CHARS))
        appendLine()
    }

    appendLine("Compact source digest:")
    appendInternetSearchSourceDigest(sources, INTERNET_SEARCH_MAX_SOURCE_TEXT_FOR_RESCUE_PROMPT)
}.trim()

internal fun internetSearchPromptSpec(
    mode: ToolInternetSearch.SearchMode,
): InternetSearchPromptSpec = when (mode) {
    ToolInternetSearch.SearchMode.QUICK_ANSWER -> InternetSearchPromptSpec(
        systemPrompt = INTERNET_SEARCH_QUICK_ANSWER_SYSTEM_PROMPT,
        rescueSystemPrompt = INTERNET_SEARCH_QUICK_ANSWER_RESCUE_SYSTEM_PROMPT,
        maxTokens = 900,
        rescueMaxTokens = 1_100,
    )

    ToolInternetSearch.SearchMode.RESEARCH -> InternetSearchPromptSpec(
        systemPrompt = INTERNET_SEARCH_RESEARCH_SYSTEM_PROMPT,
        rescueSystemPrompt = INTERNET_SEARCH_RESEARCH_RESCUE_SYSTEM_PROMPT,
        maxTokens = 3_200,
        rescueMaxTokens = 3_600,
    )
}

private fun StringBuilder.appendInternetSearchSourceDigest(
    sources: List<InternetSearchCollectedSource>,
    ...
    sources.forEach { source ->
        ...
        if (source.snippet.isNotBlank()) {
            ...
            appendLine(source.snippet.take(INTERNET_SEARCH_MAX_SOURCE_SNIPPET_CHARS))
            appendLine("UNTRUSTED_SNIPPET>>>")
        }
...
private const val INTERNET_SEARCH_UNTRUSTED_WEB_SOURCE_RULES = """
...
private const val INTERNET_SEARCH_STRATEGY_SYSTEM_PROMPT = """
...
private const val INTERNET_SEARCH_QUICK_ANSWER_SYSTEM_PROMPT = """
...
""" + INTERNET_SEARCH_UNTRUSTED_WEB_SOURCE_RULES

private const val INTERNET_SEARCH_QUICK_ANSWER_RESCUE_SYSTEM_PROMPT = """
...
private const val INTERNET_SEARCH_RESEARCH_SYSTEM_PROMPT = """
...
private const val INTERNET_SEARCH_RESEARCH_RESCUE_SYSTEM_PROMPT = """
...
internal val internetSearchStrategySystemPrompt: String = INTERNET_SEARCH_STRATEGY_SYSTEM_PROMPT
