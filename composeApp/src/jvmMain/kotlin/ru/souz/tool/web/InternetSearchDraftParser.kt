...
private const val INTERNET_SEARCH_MAX_EXECUTIVE_SUMMARY_CHARS = 1_600
private const val INTERNET_SEARCH_MIN_RESEARCH_FREEFORM_CHARS = 280
        ...
        return when (mode) {
            ...
            ToolInternetSearch.SearchMode.RESEARCH -> {
                ...
                val looksLikeReport = cleaned.contains("\n## ") ||
                    cleaned.contains("\n# ") ||
                    paragraphs.size >= 3
                if (!looksLikeReport && cleaned.length < INTERNET_SEARCH_MIN_RESEARCH_FREEFORM_CHARS) {
                    return null
                }
                InternetSearchSynthesisDraft(
                    answer = extractInternetSearchExecutiveSummary(cleaned),
                    reportMarkdown = cleaned,
                    usedSourceIndexes = parseInlineSourceIndexes(cleaned, sources),
                )
            }
...
internal fun InternetSearchSynthesisDraft.isGrounded(): Boolean =
    !answer.isNullOrBlank() && usedSourceIndexes.isNotEmpty()

internal fun extractInternetSearchExecutiveSummary(reportMarkdown: String): String {
    ...
    return paragraphs
        ...
        .take(INTERNET_SEARCH_MAX_EXECUTIVE_SUMMARY_CHARS)
        .ifBlank { reportMarkdown.trim().take(INTERNET_SEARCH_MAX_EXECUTIVE_SUMMARY_CHARS) }
}
...
private fun InternetSearchSynthesisDraft.normalize(
    ...
    val resolvedAnswer = normalizedAnswer ?: when (mode) {
        ...
        ToolInternetSearch.SearchMode.RESEARCH -> normalizedReport?.let(::extractInternetSearchExecutiveSummary)
    }
