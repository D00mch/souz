...
suspend fun resolveTelegramChatCandidate(
    telegramService: TelegramService,
    selectionBroker: TelegramChatSelectionBroker?,
    rawChatName: String,
    ...
    return when (val resolution = telegramService.resolveChatTarget(rawChatName)) {
        ...
        is TelegramChatLookupResult.Ambiguous -> {
            val broker = selectionBroker ?: throw BadInputException(
                "Multiple Telegram chats match '$rawChatName'. Ask the user to clarify the chat."
            )
            val selectedChatId = broker.requestSelection(
                query = rawChatName,
...
suspend fun resolveTelegramContactCandidate(
    telegramService: TelegramService,
    selectionBroker: TelegramContactSelectionBroker?,
    ...
    return when (val resolution = telegramService.resolveContactTarget(rawTargetName)) {
        ...
        is TelegramContactLookupResult.Ambiguous -> {
            val broker = selectionBroker ?: throw BadInputException(
                "Multiple Telegram contacts match '$rawTargetName'. Ask the user to clarify the contact."
            )
            val selectedUserId = broker.requestSelection(
                query = rawTargetName,
                candidates = resolution.candidates,
            )
