...
private fun callLlmMock(ctx: AgentContext<String>): AgentContext<String> {
    ...
    if (text == "What is the weather today?") return ctx.map { "composeApp/src/jvmMain/kotlin/ru.gigadesk/tooloseApp/src/jvmMain/kotlin/ru.gigadesk/tool" }
...
suspend fun main() {
    ...
    val graph = buildGraph {
        ...
        llmCallNode.edgeTo { ctx ->
            when (ctx.input) {
                "composeApp/src/jvmMain/kotlin/ru.gigadesk/tooloseApp/src/jvmMain/kotlin/ru.gigadesk/tool" -> llmToolUseNode
                else -> userOutputNode
            }
        }
