...
- **Shared sandbox abstraction for tools and skills** in `:runtime`: local and Docker-backed sandbox filesystem/path/process contracts now sit under `runtime/src/main/kotlin/ru/souz/runtime/sandbox/`, so file tools and skill bundle loading can stay unaware of whether they run on the host or inside a persistent mounted Docker sandbox.
