...
- **HTTP backend agent runtime** in `:backend` with trusted-proxy `/v1/**` APIs for bootstrap, per-user settings, chats, chat messages, option answers, execution cancellation, event replay, and per-chat WebSocket streams plus legacy/debug `POST /agent`; stage-10 supports in-memory, filesystem, and Postgres product repositories for chats/messages/agent state/executions/options/events/tool-calls, resolves effective settings per opaque trusted user identity, enforces ownership on chat/execution/option resources, persists explicit `AgentExecution` lifecycle for every `/v1/chats/{chatId}/messages` turn, honours effective `streamingMessages` in the shared agent runtime, writes request-scoped runtime/message/execution/option events into `AgentEventRepository`, stores tool call audit rows with redacted/truncated previews only, broadcasts the persisted `AgentEvent` envelope through an in-process event bus, supports capped `GET /v1/chats/{chatId}/events?afterSeq=` and capped `WS /v1/chats/{chatId}/ws?afterSeq=` replay/live flows, clamps chat/message/event list limits to hard maximums, switches initial `/messages` turns into async `running` responses only when both effective `streamingMessages` and `wsEvents` are enabled, persists heavy `Option` rows on `option.requested`, resumes the same `executionId` after `POST /v1/options/{optionId}/answer`, keeps synthetic continuation input inside backend/agent state only, uses optimistic locking on `agent_conversation_state` with `state_conflict` failure semantics, keeps durable DB-backed event replay optional behind `SOUZ_FEATURE_DURABLE_EVENT_REPLAY`, and preserves the legacy `/agent` request-scoped runtime path for UUID-based internal calls.
