...
- **HTTP backend agent runtime** in `:backend` with trusted-proxy `/v1/**` APIs for bootstrap, first-run onboarding state/completion, per-user settings, chats, chat rename/archive lifecycle, chat messages, Telegram bot chat bindings, option answers, execution cancellation, event replay, and per-chat WebSocket streams; stage-10 supports in-memory, filesystem, and Postgres product repositories for chats, messages, agent state, executions, options, events, tool-calls, and Telegram bot bindings, auto-provisions a local `users` namespace immediately after trusted proxy auth, resolves effective settings plus persisted onboarding completion per opaque trusted user identity, now exposes public `interfaceLanguage`, `requestTimeoutMillis`, and `useFewShotExamples` through bootstrap/onboarding/settings responses without changing existing envelopes, normalizes missing settings, legacy partial settings payloads, and invalid provider-key rows into stable bootstrap/onboarding responses, enforces ownership on chat/execution/option resources, persists explicit `AgentExecution` lifecycle for every `/v1/chats/{chatId}/messages` turn, honours effective `streamingMessages` in the shared agent runtime, applies effective request-scoped provider timeouts plus few-shot-example toggling in backend model/tool setup, keeps backend Telegram bot binding routes and poller disabled by default unless `ENABLE_BACKEND_TG_FEATURE=true`, validates Telegram bot tokens via `getMe`, encrypts stored Telegram bot tokens with `TELEGRAM_TOKEN_ENCRYPTION_KEY`, keeps new bindings in pending-link state until the first private Telegram user claims them, accepts later Telegram turns only from that linked private account, forces Telegram turns through non-streaming `AgentExecutionService.executeChatTurn(...)`, sends the final assistant response back to Telegram in 4096-char chunks with a short fallback when needed, emits durable `message.created` for Telegram-originated user messages so they appear live in the UI and carries `clientMessageId` when present for frontend reconciliation, long-polls `getUpdates` under per-binding leases for multi-instance safety, bounds polling fan-out with a semaphore, writes durable request-scoped runtime/message/execution/option events into `AgentEventRepository`, stores tool call audit rows with redacted/truncated previews only, broadcasts both persisted durable events and live-only ephemeral events through an in-process event bus, keeps `MESSAGE_DELTA` live-only so it is never assigned a fake durable `seq` or returned by replay, supports capped `GET /v1/chats/{chatId}/events?afterSeq=` and capped `WS /v1/chats/{chatId}/ws?afterSeq=` replay/live flows, clamps chat/message/event list limits to hard maximums, switches initial `/messages` turns into async `running` responses only when both effective `streamingMessages` and `wsEvents` are enabled, persists heavy `Option` rows on `option.requested`, resumes the same `executionId` after `POST /v1/options/{optionId}/answer`, keeps synthetic continuation input inside backend runtime state only, uses optimistic locking on `agent_conversation_state` with `state_conflict` failure semantics, and keeps durable DB-backed event replay optional behind `SOUZ_FEATURE_DURABLE_EVENT_REPLAY`.
