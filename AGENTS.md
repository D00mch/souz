...
- **Shared sandbox abstraction for tools and skills** in `:sharedLogic`: Android/Desktop shared contracts live under `sharedLogic/src/commonJvmMain/kotlin/ru/souz/runtime/sandbox/`, with Android app-private direct filesystem sandbox storage, POSIX `/system/bin/sh` shell skill execution in `androidMain` (not GNU Bash), and embedded Chaquopy Python skill execution through a private Android app service process, plus local/Docker process sandboxes in `jvmMain`.
...
- **Android chat-agent entry point** using Android-specific Material screens from `:sharedUI` `androidMain` backed by the shared `MainViewModel` and `SettingsViewModel`, with Markdown/code chat rendering, chat search, thinking panel, tool permission/review dialogs, Android Keystore-backed provider key settings from `:sharedLogic`, OpenAI Codex device-code account login, app-private direct sandbox filesystem plus SQLite vector storage, shared remote LLM providers, portable runtime tools, Android app-control tools for package/app discovery, opening launcher/deep-link targets, active media-session commands, and best-effort input, filesystem-backed skill registry, active-skill POSIX shell and embedded Python command execution through `RunSkillCommand`, and `GraphBasedAgent` through `AgentFacade` with no-op Android host adapters for desktop-only services. File attachment picking remains a desktop-only host feature.
...
│   ├── src/androidMain/kotlin/ru/souz/     # Android settings, app-control tools, plus SQLite runtime sandbox implementations
...
- Android app: `./gradlew :androidApp:assembleDebug` builds the Android entry point. Default Android builds include Chaquopy Python 3.11 stdlib and app Python sources without requiring host Python, and package `armeabi-v7a`, `arm64-v8a`, and `x86_64` native runtimes; pass `-Psouz.android.bundlePythonRequirements=true` to opt into bundled document/data Python packages (`lxml`, `Pillow`, `XlsxWriter`, and `python-pptx`), which requires Python 3.11 via `souz.android.buildPython` / `SOUZ_ANDROID_BUILD_PYTHON` or a discoverable install. The Android host provides one Kodein graph with Android settings/runtime bindings, and `:sharedUI` `androidMain` renders Android-specific chat/settings screens backed by common ViewModels without file attachment picking. Android uses Android Keystore-encrypted provider key storage via SharedPreferences, SQLite-backed vector storage, app-private direct sandbox filesystem storage, filesystem-backed skill registry, Android-only `APPLICATIONS`/`DESKTOP` tools for launchable package discovery, app/deep-link opening, media controls, and best-effort input, active-skill POSIX `/system/bin/sh` command execution through `RunSkillCommand` rather than GNU Bash, embedded Chaquopy Python 3.11 skill execution in a private killable service process with stdlib plus skill-vendored pure-Python files and no runtime pip installs, and `GraphBasedAgent` chat execution through the Android-safe `:sharedLogic` LLM runtime.
