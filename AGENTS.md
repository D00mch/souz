...
- **Canonical long-term memory runtime** with shared `:agent` memory contracts, desktop SQLite canonical storage at `~/.local/state/souz/memory.db`, safe no-op fallback on desktop init failure, backend storage-mode parity (`memory`/`filesystem`/`postgres`), execution-path memory retrieval/write wiring in both desktop and backend, minimal trusted-user backend memory inspection/manage routes, diagnostics tables, graph read model support, and a derived embedding/Lucene projection rebuilt from canonical storage under `~/.local/state/souz/memory-vector-index/` on desktop.
...
├── sharedLogic/                            # Shared JVM runtime, sandbox/skills, provider clients, and backend-safe tools
