...
- **Telegram chat lookup/history refresh**: Telegram chat cache now preloads up to 500 main-list chats, falls back to TDLib server-side chat search when local cache misses, and paginates `GetChatHistory` calls so "read chat" requests can force-refresh and cache larger recent windows (default 100, up to 500 messages) instead of relying on a single short TDLib page.
