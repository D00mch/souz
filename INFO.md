...
- **Web research tool**: new `WebResearch` tool can run general web search, image discovery, page text extraction, optional local image download for use in presentations or any other workflow, retries weak/empty searches through multiple query variants before giving up, and now caps image-search fan-out with per-request timeouts so slow sites do not stall the whole research step for minutes.
