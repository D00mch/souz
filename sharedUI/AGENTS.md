...
- Keep `commonJvmMain` free of AWT/Swing, desktop window APIs, native-model classes, and OS-bound side effects. Express platform work through host ports with Android-safe or no-op implementations where appropriate.
- Put Android layouts and routing in `androidMain`; put desktop windows, pickers, drag/drop, native integrations, and desktop-only screens in `jvmMain`.
