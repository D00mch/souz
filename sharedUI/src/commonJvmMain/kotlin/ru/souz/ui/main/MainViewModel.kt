...
class MainViewModel(
    @Suppress("unused")
    // Keep the scope token reachable while the ViewModel is alive; WeakContextScope uses weak keys.
    private val scopeToken: MainViewModelScope,
    private val agentFacade: AgentFacade,
    private val backgroundIndexRefresher: BackgroundIndexRefresher,
    private val settingsProvider: SettingsProvider,
    private val llmBuildProfile: LlmBuildProfile,
    private val localModelUiHost: LocalModelUiHost,
    private val pathOpener: PathOpener,
    private val chatCommandInputSource: ChatCommandInputSource,
    ...
    private val attachmentsUseCase: ChatAttachmentsUseCase,
    override val ioDispatchers: CoroutineDispatcher = scopeToken.ioDispatcher,
    ...
    init {
        ...
        viewModelScope.launch(start = CoroutineStart.UNDISPATCHED) { collectAmbientModeState() }
        ...
        viewModelScope.launch {
            ...
            setState {
                copy(
                    ...
                    isSandboxed = permissionPromptService.isSandboxed,
                )
            }
        }
        ...
        viewModelScope.launch {
            chatCommandInputSource.cleanCommands.collect {
    ...
    override suspend fun handleEvent(event: MainEvent) {
        when (event) {
            ...
            is MainEvent.OpenPath -> {
                pathOpener.openPath(event.path)
                    .onFailure { error ->
            ...
            is MainEvent.SendChatMessage -> vmLaunch {
                ...
                val capturedRequestId =
                    (reviewedVoiceDraft?.route as? VoiceInputRoute.ActiveRunContinuation)?.requestId
                if (capturedRequestId != null) {
                    submitActiveRunInput(
                        input = inputText,
                        expectedRequestId = capturedRequestId,
                ...
                if (
                    reviewedVoiceDraft == null &&
                    currentState.isProcessing &&
                    currentState.supportsActiveRunInput
                ) {
    ...
    private suspend fun handleRecognizedVoiceInput(input: RecognizedVoiceInput) {
        ...
        val accepted = when (val route = input.route) {
    ...
    private suspend fun collectAmbientModeState() {
        ambientModeController.modeState.collect { mode ->
            val voiceDisabledReason = if (mode.isVoiceInputBlocked()) {
    ...
    private suspend fun collectAmbientSuggestions() {
        ambientModeController.suggestions.collect { suggestions ->
            setState {
                copy(
                    ambientSuggestions = suggestions.map { it.toUiModel() },
                )
            }
        }
    }
    ...
    private fun CoroutineScope.launchDbSetup(repo: BackgroundIndexRefresher): Job = launch(ioDispatchers) {
        while (isActive) {
    ...
    private suspend fun updateChatModel(modelAlias: String) {
        ...
        val downloadPrompt = localModelUiHost.downloadPromptFor(model)
        if (downloadPrompt != null) {
            setState {
                copy(
                    localModelDownloadPrompt = downloadPrompt,
    ...
    override fun onCleared() {
        ...
        permissionsUseCase.rejectPendingSelectionDialog(
            sourceId = pendingSelectionSourceId,
